...
class FlaskTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        the_application.signup("homie", "homie@gmail.com", "duff")
    ...
    def test_signup(self):
        ...
        with self.tester as the_tester:
            the_tester.post("/signup", data=dict(username="marge_s", email="marge_s@gmail.com", password="maggie"),
                            follow_redirects=True)
            self.assertEqual(
                "marge_s", session['username'],
                msg="The session should contain a username `marge_s`")

    def test_login(self):
        """This method tests if the application logs in users correctly"""
        with self.tester as the_tester:
            the_tester.post("/login", data=dict(username="homie", password="duff"),
                            follow_redirects=True)
            self.assertTrue(the_application.logged_in["homie"],
                            msg="The session user `homie` should be logged in")
    ...
    def test_logout(self):
        ...
        with self.tester as the_tester:
            ...
            self.assertFalse(the_application.logged_in["homie"],
                             msg="The session user `homie` should be logged out")
    ...
    def test_create_shopping_list(self):
        ...
        with self.tester as the_tester:
            ...
            self.assertIn("List 1", the_application.get_all_lists("homie").keys(),
        ...
        with self.tester as the_tester:
            ...
            the_tester.post("/edit_shopping_list",
                            data=dict(old_title="List 1",
                                      new_title="New List"),
                            follow_redirects=True)
            self.assertIn("New List", the_application.get_all_lists("homie").keys(),
                          msg="There should be a list named `List 1` in the user lists")
            self.assertNotIn("List 1", the_application.get_all_lists("homie").keys(),
                             msg="There should be a list named `List 1` in the user lists")
    ...
    def test_add_item(self):
        ...
        with self.tester as the_tester:
            ...
            self.assertIn("Item 1", the_application.users["homie"].shopping_lists["List 1"].items.keys(),
    ...
    def test_edit_item(self):
        ...
        with self.tester as the_tester:
            ...
            the_tester.post("/edit_item",
                            ...
                            follow_redirects=True)
            self.assertIn("New Item", the_application.users["homie"].shopping_lists["List 1"].items.keys(),
                          msg="There should be an item named `New Item` in the user list items")
            self.assertNotIn("Item 1", the_application.users["homie"].shopping_lists["List 1"].items.keys(),
    ...
    def test_check_item_toggle(self):
        ...
        with self.tester as the_tester:
            ...
            the_tester.get("/check_item_toggle?list_title='List 1'&name='Item 1'&new_status='false'",
                           follow_redirects=True)
            self.assertFalse(the_application.users["homie"].shopping_lists["List 1"].items["Item 1"].status,
